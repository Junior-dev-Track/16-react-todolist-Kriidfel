...
function TodoItem({ todo, onCheck, onDelete, onEdit }) {
  return (
      <li className={"todo"}>
          <input className={"check"} type="checkbox" checked={todo.checked} onChange={onCheck}/>
          <p>{todo.text}</p>
    ...
    <>
      ...
      <ul className={"allTodo"}>
        ...
        {todos.map((todo, index) => (
          <TodoItem key={index} todo={todo} onCheck={() => toggleTodo(index)} onDelete={() => deleteTodo(index)} onEdit={()=> editTodo(index)} />
        ))}
      </ul>
    </>
  );
